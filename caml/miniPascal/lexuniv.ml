...
let rec lire_mot position flux =
  ...
  |[< '(`A`..`Z` | `a`..`z` | `0`..`9` | `_` | `'`
	...
	|`É`|`À`|`È`|`Ù`|`Â`|`Ê`|`Î`|`Ô`|`Û`|`Ë`|`Ï`|`Ü`|`Ç` as c) >] ->
    if position < string_length tampon then tampon.[position] <- c; lire_mot (position + 1) flux
...
let rec lire_symbole position flux =
  ...
  |[< '(`!`|`$`|`%`|`&`|`*`|`+`|`-`|`.`|`/`|`:`
    ...
    if position < string_length tampon then
      tampon.[position] <- c; lire_symbole (position + 1) flux
  |[<>] -> sub_string tampon 0 (min position (string_length tampon));;
...
let rec lire_lexeme table flux =
  match flux with
  | [< '(` `|`\n`|`\r`|`\t`) >] -> lire_lexeme table flux
  | [< '`#` >] ->lire_commentaire flux; lire_lexeme table flux
  |[< '(`A`..`Z` | `a`..`z`
	|`é`|`à`|`è`|`ù`|`â`|`ê`|`î`|`ô`|`û`|`ë`|`ï`|`ü`|`ç`
	|`É`|`À`|`È`|`Ù`|`Â`|`Ê`|`Î`|`Ô`|`Û`|`Ë`|`Ï`|`Ü`|`Ç`
	as c) >] ->
    tampon.[0] <- c;mc_ou_ident table (lire_mot 1 flux)
  |[< '(`!`|`$`|`%`|`&`|`*`|`+`|`.`|`/`|`:`
	|`;`|`<`|`=`|`>`|`?`|`@`|`^`| `|` |`~` as c) >] -> tampon.[0] <- c; mc_ou_ident table (lire_symbole 1 flux)
  |[< '(`0`..`9` as c) >] -> Entier(lire_entier (int_of_char c - 48) flux)
  |[< '`-` >] ->
    begin match flux with
	  |[< '(`0`..`9` as c) >] -> Entier(- (lire_entier (int_of_char c - 48) flux))
	  |[< >] -> tampon.[0] <- `-`; mc_ou_ident table (lire_symbole 1 flux)
    end
