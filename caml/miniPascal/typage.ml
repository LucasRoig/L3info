...
| Tableau_attendu             (* [..] applique a un non-tableau *)
...
let verifie_non_tableau message = function
  | Array(inf, sup, elements) -> raise(Erreur_typage(Tableau_interdit message))
  | _ -> ();;

let rec type_expr env = function
  | Constante(Entiere n) -> Integer
  | Constante(Booleenne b) -> Boolean
  | Variable nom_var -> cherche_variable nom_var env
  ...
  | Op_unaire(op, arg) ->
     ...
     verifie_type ("l argument de " ^ op) type_arg (type_expr env arg);
     type_res
  | Op_binaire(op, arg1, arg2) ->
     let (type_arg1, type_arg2, type_res) = type_op_binaire op in
     verifie_type ("le premier argument de " ^ op) type_arg1 (type_expr env arg1);
     verifie_type ("le deuxi`eme argument de " ^ op) type_arg2 (type_expr env arg2);
     type_res
